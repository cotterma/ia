            ...
            if intent == "inform_gni":
                ...
                if not filtered_data.empty:
                    if "maximum" in user_message:
                        max_gni = filtered_data["GNI_per_capita"].max()
                        dispatcher.utter_message(f"The maximum GNI per capita for {country} from {year_start} to {year_end} is {max_gni:.3f}")
                    elif "minimum" in user_message:
                        min_gni = filtered_data["GNI_per_capita"].min()
                        dispatcher.utter_message(f"The minimum GNI per capita for {country} from {year_start} to {year_end} is {min_gni:.3f}")
                    elif "average" in user_message:
                        avg_gni = filtered_data["GNI_per_capita"].mean()
                        dispatcher.utter_message(f"The average GNI per capita for {country} from {year_start} to {year_end} is {avg_gni:.3f}")
                else:
                    dispatcher.utter_message(f"No data found for {country} from {year_start} to {year_end}.")
            else:
                dispatcher.utter_message("I can only respond to GNI per capita queries. Please rephrase your question.")
        elif year:
            if intent == "inform_gni":
                ...
                if not filtered_data.empty:
                    if "maximum" in user_message:
                        max_gni = filtered_data["GNI_per_capita"].max()
                        dispatcher.utter_message(f"The maximum GNI per capita in {year} is {max_gni:.3f}")
                    elif "minimum" in user_message:
                        min_gni = filtered_data["GNI_per_capita"].min()
                        dispatcher.utter_message(f"The minimum GNI per capita in {year} is {min_gni:.3f}")
                    elif "average" in user_message:
                        avg_gni = filtered_data["GNI_per_capita"].mean()
                        dispatcher.utter_message(f"The average GNI per capita in {year} is {avg_gni:.3f}")
                else:
